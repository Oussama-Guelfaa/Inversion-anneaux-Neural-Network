...
def train_model(X_train, y_train, X_val, y_val, noise_level=5):
    ...
    model_path = "models/model_retrained_5percent.pth"
...
def create_analysis_plots(y_test, y_pred, history, range_metrics):
    ...
    plt.tight_layout()
    plt.savefig('plots/retrained_model_analysis.png', dpi=300, bbox_inches='tight')
    plt.close()

    print("✅ Graphiques sauvegardés dans plots/retrained_model_analysis.png")
...
def save_results(metrics, y_test, y_pred, training_time):
    ...
    summary = {
        ...
        'augmentation': 'interpolation_factor_2',
        'noise_level': '5%',
    ...
    with open('results/retrained_model_summary.json', 'w') as f:
    ...
    predictions_df.to_csv('results/retrained_predictions.csv', index=False)
...
def main():
    ...
    # 2. Augmentation par interpolation
    print("\n📈 AUGMENTATION DES DONNÉES")
    X_augmented, y_augmented = augment_data_by_interpolation(
        intensity_profiles, gap_values, factor=2
    )
    ...
    print(f"   Après augmentation: {len(X_augmented)} échantillons")
    print(f"   Plage étendue: 0.005 - 3.000 µm (vs précédent)")
