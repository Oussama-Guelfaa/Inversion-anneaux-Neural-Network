...
def calculate_tolerance_accuracy(y_true, y_pred, tolerance=0.01):
    """
    ...
    Args:
        ...
        tolerance: Tolérance acceptable (défaut: 0.01)
...
def evaluate_with_tolerance(model, X_test, y_test_scaled, y_test_original, scaler_L, scaler_gap, tolerance=0.01):
    """Évalue le modèle avec métriques classiques + tolérance."""
    
    print(f"\n=== ÉVALUATION AVEC TOLÉRANCE (±{tolerance}) ===")
    ...
    # === NOUVELLES MÉTRIQUES AVEC TOLÉRANCE ===
    accuracy_L_tolerance = calculate_tolerance_accuracy(y_test_original[:, 0], y_pred_rounded[:, 0], tolerance)
    accuracy_gap_tolerance = calculate_tolerance_accuracy(y_test_original[:, 1], y_pred_rounded[:, 1], tolerance)
    accuracy_global_tolerance = calculate_tolerance_accuracy(y_test_original.flatten(), y_pred_rounded.flatten(), tolerance)
    
    # Compter les prédictions parfaites par paramètre
    perfect_L = np.sum(np.abs(y_test_original[:, 0] - y_pred_rounded[:, 0]) <= tolerance)
    perfect_gap = np.sum(np.abs(y_test_original[:, 1] - y_pred_rounded[:, 1]) <= tolerance)
    perfect_both = np.sum((np.abs(y_test_original[:, 0] - y_pred_rounded[:, 0]) <= tolerance) & 
                         (np.abs(y_test_original[:, 1] - y_pred_rounded[:, 1]) <= tolerance))
    ...
    print(f"\nMÉTRIQUES AVEC TOLÉRANCE (±{tolerance}):")
    print(f"  Accuracy L_ecran: {accuracy_L_tolerance:.2f}% ({perfect_L}/{len(y_test_original)} échantillons)")
    print(f"  Accuracy gap: {accuracy_gap_tolerance:.2f}% ({perfect_gap}/{len(y_test_original)} échantillons)")
    print(f"  Accuracy globale: {accuracy_global_tolerance:.2f}%")
    print(f"  Prédictions parfaites (L_ecran ET gap): {perfect_both}/{len(y_test_original)} ({perfect_both/len(y_test_original)*100:.1f}%)")
    ...
    print(f"    Dans tolérance: {np.sum(errors_L <= tolerance)}/{len(errors_L)}")
    
    print(f"  Erreurs gap:")
    print(f"    Min: {errors_gap.min():.6f} µm")
    print(f"    Max: {errors_gap.max():.6f} µm")
    print(f"    Moyenne: {errors_gap.mean():.6f} µm")
    print(f"    Dans tolérance: {np.sum(errors_gap <= tolerance)}/{len(errors_gap)}")
    
    success_r2 = r2_global > 0.8
    success_tolerance = accuracy_global_tolerance > 80.0  # 80% des prédictions dans la tolérance
    
    print(f"\nOBJECTIFS:")
    print(f"  R² > 0.8: {'✅ ATTEINT' if success_r2 else '❌ NON ATTEINT'} (R² = {r2_global:.6f})")
    print(f"  Accuracy > 80%: {'✅ ATTEINT' if success_tolerance else '❌ NON ATTEINT'} (Accuracy = {accuracy_global_tolerance:.2f}%)")
    ...
    return {
        ...
        'accuracy_L_tolerance': accuracy_L_tolerance,
        'accuracy_gap_tolerance': accuracy_gap_tolerance,
        'accuracy_global_tolerance': accuracy_global_tolerance,
        'perfect_L': perfect_L, 'perfect_gap': perfect_gap, 'perfect_both': perfect_both,
...
def load_test_data(scaler_X, scaler_L, scaler_gap):
...
def main():
    ...
    print("="*80)
    print("🎯 NEURAL NETWORK 06-06-25 AVEC ÉVALUATION PAR TOLÉRANCE")
    print("Tolérance: ±0.01 pour considérer une prédiction comme correcte")
    print("="*80)
    ...
    X_test_scaled, y_test_original = load_test_data(scaler_X, scaler_L, scaler_gap)
    ...
    # 5. Évaluation avec tolérance
    y_test_scaled_dummy = np.zeros((len(y_test_original), 2))  # Pas utilisé dans cette version
    metrics = evaluate_with_tolerance(model, X_test_scaled, y_test_scaled_dummy, y_test_original, 
                                    scaler_L, scaler_gap, tolerance=0.01)
    ...
    print(f"\nPERFORMANCES AVEC TOLÉRANCE (±0.01):")
    print(f"  Accuracy L_ecran: {metrics['accuracy_L_tolerance']:.2f}%")
    print(f"  Accuracy gap: {metrics['accuracy_gap_tolerance']:.2f}%")
    print(f"  Accuracy globale: {metrics['accuracy_global_tolerance']:.2f}%")
    print(f"  Prédictions parfaites: {metrics['perfect_both']}/{len(y_test_original)} ({metrics['perfect_both']/len(y_test_original)*100:.1f}%)")
